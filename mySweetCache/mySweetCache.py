...
def cache(
    ...
    def wrapper(MSC_name: Optional[str], fun: Callable):
        ...
        def TO_RETURN(*args, use_cache: bool = SETUP["MSC_USE_CACHE"]):
            save_helper = SaveHelper(is_string=is_string)
            if MSC_name in os.listdir(SETUP["CACHE_FILES"]) and use_cache:
                return save_helper.read_from_file(os.sep.join([SETUP["CACHE_FILES"], MSC_name]))
            ret = fun(*args)
            save_helper.save_to_file(ret, os.sep.join([SETUP["CACHE_FILES"], MSC_name]), MSC_name)
            return ret
